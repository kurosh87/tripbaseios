...
extension DB {
    ...
    public func signOut() throws {
        ...
        do {
            ...
            Analytics.capture(.error, id: "sign_out", longDescription: "Error signing out: \(error.localizedDescription)", source: .auth)
    ...
    public func newDisplayName(_ name: String) async throws {
        ...
        do {
            ...
            Analytics.capture(.success, id: "new_display_name", longDescription: "Display name updated to \(name).", source: .auth)
        } catch {
...
extension DB {
    ...
    private func _consequenceExecutionForNotSignedIn(_ consequence: NotSignedInConsequence) {
        if consequence == .showInAppNotification {
            showInAppNotification(.error, content: .init(title: "Sign In Required", message: "Can't perform this action"), size: .compact)
        } else {
            showSignInSheet()
        }
    }
